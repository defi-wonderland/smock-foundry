import {Ast, ContractDefinitionNode, FunctionDefinitionNode, VariableDeclarationNode} from "./types";
...
export const getConstructor = (sourceFilePath: string): Promise<string> => {
  return new Promise((resolve, reject) => {
    ...
    const contractNode: ContractDefinitionNode = ast.nodes.find((node): node is ContractDefinitionNode => node.nodeType === "ContractDefinition");
    ...
    const functionNodes: FunctionDefinitionNode[] = contractNode.nodes.filter(
      (node): node is FunctionDefinitionNode => node.nodeType === "FunctionDefinition"
    );
    // Find the node from the functionNodes that is the constructor
    const constructorNode: FunctionDefinitionNode = functionNodes.find((node) => node.kind === 'constructor');

    const parameters: VariableDeclarationNode[] = constructorNode.parameters.parameters;
    ...
    for (const parameter of parameters) {
      const storageLocation = parameter.storageLocation === 'memory' || parameter.storageLocation === 'calldata'
        ? parameter.storageLocation
        : '';
    
      const typeName: string = parameter.typeDescriptions.typeString.replace(/contract /g, '');
      const parameterString: string = `${typeName} ${storageLocation} ${parameter.name}`;
